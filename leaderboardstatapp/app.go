...
//func Setup(ctx context.Context, config Config, conn *database.Database, logger *slog.Logger, pub eventpublisher.Publisher) Application {
//
//	categoryRepo := repository.NewCategoryRepo(conn.DB)
//	categoryValidator := category.NewValidator()
//	categorySvc := category.NewService(categoryRepo, categoryValidator)
//	categoryHandler := http.NewCategoryHandler(categorySvc)
