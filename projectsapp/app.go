...
import (
	...
	"github.com/gocasters/rankr/projectsapp/delivery/http"
	"github.com/gocasters/rankr/projectsapp/interfaces"
	"github.com/gocasters/rankr/projectsapp/repository"
	"github.com/gocasters/rankr/projectsapp/services"
)

type Application struct {
	ProjectRepo interfaces.IProjectRepository
	VcsRepoRepo interfaces.VcsRepoRepository
...
func Setup(
	...
	projectRepo := repository.NewProjectRepositoryPG(postgresConn.Pool)
	vcsRepoRepo := repository.NewVcsRepoRepositoryPG(postgresConn.Pool)

	validator := services.NewValidator(projectRepo, vcsRepoRepo)

	projectService := services.NewProjectService(projectRepo, validator)
	vcsRepoService := services.NewVcsRepoService(vcsRepoRepo, projectRepo, validator)
	...
	projectHandler := http.NewHandler(projectService, vcsRepoService, logger)
