...
func TestService_BroadcastEvent(t *testing.T) {
	...
	client1.Subscriptions[constant.TopicTaskCreated] = true
	service.RegisterClient(client1)

	client2 := createTestClient("client-2")
	client2.Subscriptions[constant.TopicTaskCreated] = true
	service.RegisterClient(client2)

	client3 := createTestClient("client-3")
	client3.Subscriptions[constant.TopicContributorCreated] = true
	...
	t.Run("should broadcast event to subscribed clients", func(t *testing.T) {
		...
		case msg := <-client1.Send:
			assert.NotNil(t, msg)
			assert.Contains(t, string(msg), constant.TopicTaskCreated)
	...
	t.Run("should broadcast to different topic", func(t *testing.T) {
		...
		case msg := <-client3.Send:
			assert.NotNil(t, msg)
			assert.Contains(t, string(msg), constant.TopicContributorCreated)
		case <-time.After(100 * time.Millisecond):
...
func TestService_ConcurrentOperations(t *testing.T) {
	...
	for i := 0; i < numClients; i++ {
		go func(id int) {
			defer wg.Done()
			req := SubscribeRequest{
				Topics: []string{constant.TopicTaskCreated, constant.TopicTaskUpdated},
			}
...
func TestService_SubscriptionPersistence(t *testing.T) {
	...
	retrieved, ok := store.GetClient("client-1")
	require.True(t, ok)
	assert.True(t, retrieved.Subscriptions[constant.TopicTaskCreated])
	assert.True(t, retrieved.Subscriptions[constant.TopicTaskUpdated])
	assert.True(t, retrieved.Subscriptions[constant.TopicContributorCreated])

	unsub := UnsubscribeRequest{
		Topics: []string{constant.TopicTaskUpdated},
	}
	...
	require.True(t, ok)
	assert.True(t, retrieved.Subscriptions[constant.TopicTaskCreated])
	assert.False(t, retrieved.Subscriptions[constant.TopicTaskUpdated])
	assert.True(t, retrieved.Subscriptions[constant.TopicContributorCreated])
}
